...
@Component({
  ...
  props: {
    ...
    tips: {
      type: String,
      default: '格式支持doc，docx，xls，xlsx，ppt，pptx，pdf，文件大小不超过10M。'
    },
    ...
    // 文件接收类型,默认只支持doc,docx,xlsx,ppt,pptx,pdf
    accept: {
      type: String,
      default: 'application/vnd.ms-excel,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    },
...
export default class BookUploader extends Vue {
  ...
  async getUploadFileConfig () {
    try {
      ...
      this.$message.error(e.message || '获取文件上传配置出错')
  ...
  handleChangeFile (event) {
    ...
    if (!file.type || this.accept.indexOf(file.type) < 0) {
      ...
      // this.$message.error('请上传doc，docx，xls，xlsx，ppt，pptx，pdf格式的文件')
      this.clearFileInput()
      return
    }
