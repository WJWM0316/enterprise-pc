    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot="header" slot-scope="{ column, $index }">
          {{fields[$index].dropdown && fields[$index].dropdown.length ? '' : column.label}}
          <el-dropdown @command="handleCommand($event, fields[$index].prop)" v-if="fields[$index].dropdown && fields[$index].dropdown.length">
            <span class="el-dropdown-link">
              {{column.label}}
              <i class="el-icon-caret-bottom el-icon--right" style="margin-left: -1px;margin-right: 2px;"></i>
            </span>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                :command="item.value"
                v-for="(item, index) in fields[$index].dropdown"
                :key="index">
                {{item.label}}
              </el-dropdown-item>
            </el-dropdown-menu>
          ...
          <el-popover
            ...
            trigger="hover"
            width="200"
            v-if="fields[$index].tooltip"
            :content="fields[$index].tooltip">
            <i class="el-icon-question" slot="reference" style="color: rgb(188, 188, 188); font-size: 16px;position: relative;top: 1px;margin-left: 2px;"></i>
          </el-popover>
        </template>
