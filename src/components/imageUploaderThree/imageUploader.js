...
import { uploadImgBs } from '@/api/common'
...
@Component({
  ...
  watch: {
    ...
    oldImgUrl (val) {
      this.previewSrc = this.oldImgUrl
    }
  }
})
export default class ImageUploader extends Vue {
  ...
  async upldImg (url) {
    try {
      const params = { img: url }
      const data = await uploadImgBs(params)
      let id = data.img.file_id.toString()
      this.previewSrc = data.img.url
      this.imgId = id
      this.$emit('input', id)
    } catch (error) {
