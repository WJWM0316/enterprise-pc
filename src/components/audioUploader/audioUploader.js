...
import { getUploadFileConfig, getBeaconUploadFileConfig, putUploadFileToOSS } from '@/api/common'
...
@Component({
  ...
  props: {
    ...
    // api接口类型：0=>使用getUploadFileConfig获取上传配置，1=>使用getBeaconUploadFileConfig接口
    apiType: {
      type: Number,
      default: 0
    },
...
export default class AudioUploader extends Vue {
  ...
  // api接口类型选择
  apiTypeOptions = {
    0: getUploadFileConfig,
    1: getBeaconUploadFileConfig
  }
    ...
    try {
      ...
      const params = {
        file: {
          ...
          running_time: this.audio.duration
        },
        file_type: this.fileTypeOptions[this.apiType]
      }
      const { oss_config: ossConfig, system_config: systemConfig } = await this.apiTypeOptions[this.apiType](params)
      this.progress = 0 // 进度重置为0
      this.uploadFile(ossConfig, systemConfig)
    } catch (e) {
      this.$message.error(e.message || '获取音频上传配置出错')
  ...
  handleChangeFile (event) {
    ...
    console.log(file.size, 1024 * 1024 * this.maxFileSize, this.maxFileSize)
    // 判断文件大小
